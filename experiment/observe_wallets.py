...
def buy_quote(asks: list[dict[str, Any]], cash: float, fee_rate: float) -> dict[str, Any]:
...
def quote_trade(
    ...
    quote = buy_quote(book.get("asks") or [], cash, fee_rate) if side == "BUY" else None
    return {
        ...
        "market": {
            ...
            "fee": {"rate": fee_rate, "exponent": fee_details.get("e"), "taker_only": fee_details.get("to")},
        },
        "book": {
    ...
    while args.max_polls == 0 or polls < args.max_polls:
        ...
        for lane in lanes:
            try:
                ...
                fetch_errors.append((lane, error))
                continue
            ...
            for trade in reversed(trades):
                ...
                if not first_run and lane_accepts(trade, lane):
                    new_trades.append((trade, lane, int(time.time() * 1000)))
        if first_run and fetch_errors:
            names = ", ".join(lane["name"] for lane, _ in fetch_errors)
        ...
        state = {"version": 1, "updated_at_ms": int(time.time() * 1000), "seen": ordered[:5000]}
